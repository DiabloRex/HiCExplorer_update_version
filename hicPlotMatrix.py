...
def plotHeatmap(ma, chrBinBoundaries, fig, position, args, cmap, xlabel=None,
    ...
    if args.region:
        ...
        print(axHeat2.get_xaxis())
        print(axHeat2.get_yaxis())
        ...
        pos = 0
        ticks = []
        chromosomeOrderString = toString(args.chromosomeOrder)

        if "@" in chromosomeOrderString:
            X_chromosomes = []
            Y_chromosomes = []
            ticks_x = []
            ticks_y = []
            ...
            for chrom in range(len(chromosomeOrderString)):
                if chromosomeOrderString[chrom] == "@":
                    pm = True
                    continue
            
                if pm:
                    Y_chromosomes.append(chromosomeOrderString[chrom])
                    k, v = pChromsomeStartEndDict[chromosomeOrderString[chrom]]
                    ticks_y.append(pos)
                    pos += v - k
                else:
                    X_chromosomes.append(chromosomeOrderString[chrom])
                    k, v = pChromsomeStartEndDict[chromosomeOrderString[chrom]]
                    ticks_x.append(pos)
                    pos += v - k
...
def main(args=None):
    ...
    if is_cooler and not args.region2 and open_cooler_chromosome_order:
        ...
        if args.chromosomeOrder:
            ...
            for chrom in range(len(chromosomeOrderString)):
                if chromosomeOrderString[chrom] == "@":
                    chrX_last = chromosomeOrderString[chrom - 1]
                    continue
                if chromosomeOrderString[chrom] in ma.chrBinBoundaries:
                    valid_chromosomes.append(chromosomeOrderString[chrom])
                else:
                    invalid_chromosomes.append(chromosomeOrderString[chrom])
        ...
        if args.region:
            ...
            if chrX_last is not None:
                start, end = ma.chrBinBoundaries[chrX_last]
                lasts, laste = ma.chrBinBoundaries[valid_chromosomes[len(valid_chromosomes) - 1]]
                idx1 = []
                start_pos1 = []
                start_pos2 = []
                start_poss = make_start_pos_array(ma)
                for i in range(end):
                    idx1.append(i)
                    start_pos1.append(start_poss[i])
                idx2 = []
                for i in range(end, laste):
                    idx2.append(i)
                    start_pos2.append(start_poss[i])
                matrix = np.asarray(ma.matrix[idx1, :][:, idx2].todense().astype(float))
            else:
                matrix = np.asarray(ma.getMatrix().astype(float))

        log.info("min: {}, max: {}\n".format(
            np.min(matrix[np.nonzero(matrix)]), np.max(matrix)))
